# Module 3, Lesson 7: Advanced Interactivity - Highlighting Paths

**Objective:**
*   Implement a mechanism to highlight entire paths (sequences of nodes and links) associated with a specific witness.
*   Add a way to reset the highlighting to show the full graph.

**Concepts Covered:**
*   Filtering D3.js selections based on data attributes.
*   Applying and removing CSS classes (`.highlighted`, `.dimmed`) to visually distinguish elements.
*   Managing graph state to toggle between highlighted and default views.
*   Event handling for clicks on legend items or nodes to trigger highlighting.

---

## Code Examples (Current State)

*Note: The code provided below reflects the state of the project files after completing Module 3, Lesson 7.*
...
<head>
    ...
    <title>Text Variant Graph - Highlighting Paths</title>
    ...
    <style>
        ...
        .node circle {
            ...
            stroke: #fff;
            stroke-width: 1.5px;
            transition: transform 0.2s ease, opacity 0.3s ease; /* Smooth hover and highlight effect */
        }
        .node:hover circle {
        ...
        .link {
            ...
            stroke-opacity: 0.6;
            stroke-width: 2px;
            transition: stroke-width 0.2s ease, stroke-opacity 0.2s ease; /* Smooth hover and highlight effect */
        }
        .link:hover {
            stroke-width: 3px;
            stroke-opacity: 1;
...
<body>
    ...
    <script>
        ...
        ]; // Hardcoded for now, will be loaded from references.json later
...
console.log("Lesson 7: Advanced Interactivity - Highlighting Paths")
...
window.renderGraph = (data) => {
    ...
    svg.selectAll(".link")
        ...
        .attr("data-witness", d => d.witnesses.join(",")) // Store witnesses for highlighting
    ...
    const nodes = svg.selectAll(".node")
        ...
        .attr("data-witness", d => d.witnesses ? d.witnesses.join(",") : "") // Store witnesses for highlighting
        .on("mouseover", (event, d) => {
            tooltip.style("opacity", 1)
        ...
        .on("click", (event, d) => {
            // When a node is clicked, highlight paths for its witnesses
            if (d.witnesses && d.witnesses.length > 0) {
                // For simplicity, if a node belongs to multiple witnesses,
                // we'll highlight based on the first one or all of them.
                // For this lesson, let's just highlight based on the first witness.
                window.toggleWitness(d.witnesses[0], null); // Pass null for clickedElement as it's a node click
            }
        });
    ...
    console.log("Graph with highlighting functionality rendered!");
}
