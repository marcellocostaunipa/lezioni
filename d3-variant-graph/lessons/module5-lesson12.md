# Module 5, Lesson 12: Advanced Data Filtering and Search

**Objective:**
*   Implement dynamic filtering of the graph based on active witnesses.
*   Add a search bar to filter nodes by text content.
*   Ensure the graph updates in real-time as filters and search terms are applied.

**Concepts Covered:**
*   Managing application state for filters (e.g., using `Set` for active witnesses).
*   Filtering D3.js data arrays based on multiple criteria.
*   Re-rendering the graph with filtered data.
*   Event listeners for input fields (`change`, `input`).

---

## Code Examples (Current State)

*Note: The code provided below reflects the state of the project files after completing Module 5, Lesson 12.*
...
<head>
    ...
    <title>Text Variant Graph - Filtering and Search</title>
...
console.log("Lesson 12: Advanced Data Filtering and Search")
...
window.renderGraph = (data) => {
  ...
  const linksGroup = svg.selectAll(".link-group").data(data.links).enter().append("g").attr("class", "link-group")
  ...
  const nodes = svg.selectAll(".node").data(data.nodes).enter().append("g").attr("class", (d) => `node ${d.type}`)
  ...
  console.log("Graph with filtering and search rendered!")
}
