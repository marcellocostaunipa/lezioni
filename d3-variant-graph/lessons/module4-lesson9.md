# Module 4, Lesson 9: Dynamic Arrow Colors and Link Offsets

**Objective:**
*   Make arrow colors dynamic, matching the color of the witness path they belong to.
*   Implement small offsets for parallel links between the same two nodes to prevent overlap.

**Concepts Covered:**
*   Dynamic attribute setting in D3.js based on data properties.
*   Calculating offsets for visual separation of overlapping elements.
*   Updating SVG `<marker>` definitions with dynamic `fill` attributes.

---

## Code Examples (Current State)

*Note: The code provided below reflects the state of the project files after completing Module 4, Lesson 9.*
...
<head>
    ...
    <title>Text Variant Graph - Dynamic Arrows and Offsets</title>
...
console.log("Lesson 9: Dynamic Arrow Colors and Link Offsets")
...
window.renderGraph = (data) => {
    ...
    console.log("Graph with dynamic arrow colors and link offsets rendered!");
}
...
window.toggleWitness = (witness, clickedElement) => {
    ...
    svg.selectAll(".link") // Select all individual link paths
