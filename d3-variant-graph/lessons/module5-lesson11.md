# Module 5, Lesson 11: Data Integration - Loading More Witnesses

**Objective:**
*   Expand the `references.json` file to include more witness data.
*   Observe how the `generate-graph-data.mjs` script automatically processes the new data to create a more complex graph structure.
*   Confirm that the D3.js rendering and interactivity functions gracefully handle the increased number of nodes and links.

**Concepts Covered:**
*   Scalability of data processing and visualization.
*   Review of dynamic node positioning and link offsetting with more data.
*   Ensuring legend and info cards update correctly with new witnesses.

---

## Code Examples (Current State)

*Note: The code provided below reflects the state of the project files after completing Module 5, Lesson 11.*
...
<head>
    ...
    <title>Text Variant Graph - More Witnesses</title>
...
<body>
    ...
    <script>
        ...
        window.addEventListener('resize', () => {
            if (allGraphData) {
                window.renderGraph(allGraphData); // Re-render graph on resize
            }
        });
    </script>
</body>
...
console.log("Lesson 11: Data Integration - Loading More Witnesses")
...
window.renderGraph = (data) => {
    ...
    svg.attr("viewBox", `0 0 ${svgIntrinsicWidth} ${svgIntrinsicHeight}`)
    ...
    defs.selectAll("marker").remove(); // Clear existing markers before redrawing
    ...
    svg.selectAll(".link-group, .node").remove();
    ...
    console.log("Graph with more witnesses rendered!");
}
...
\`\`\`json
{
  "nodes": [
    ...
    { "id": "dei_pos9", "text": "dei", "type": "variant", "witnesses": ["A"], "x": 1250, "y": 300 },
    {
      "id": "end",
      "text": "END",
      "type": "end",
