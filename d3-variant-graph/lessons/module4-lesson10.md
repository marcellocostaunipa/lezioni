# Module 4, Lesson 10: Responsive Design - SVG Resizing

**Objective:**
*   Make the SVG graph responsive to window resizing.
*   Ensure the graph scales appropriately while maintaining its aspect ratio.

**Concepts Covered:**
*   SVG `viewBox` and `preserveAspectRatio` attributes.
*   CSS `width: 100%` and `height: 100%` for SVG elements within a container.
*   JavaScript `window.addEventListener('resize')` to re-render the graph on window size changes.
*   Calculating dynamic `totalGraphWidth` in `generate-graph-data.mjs` for the `viewBox`.

---

## Code Examples (Current State)

*Note: The code provided below reflects the state of the project files after completing Module 4, Lesson 10.*
...
<head>
    ...
    <title>Text Variant Graph - Responsive Design</title>
...
console.log("Lesson 10: Responsive Design - SVG Resizing")
...
window.renderGraph = (data) => {
    ...
    svg.attr("viewBox", `0 0 ${svgIntrinsicWidth} ${svgIntrinsicHeight}`)
       .attr("preserveAspectRatio", "xMidYMid meet"); // Center the content and scale to fit
    ...
    console.log("Graph with responsive design rendered!");
}
