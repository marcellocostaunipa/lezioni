# Module 2, Lesson 5: Dynamic Node Positioning and Data Generation

**Objective:**
*   Introduce a Node.js script (`generate-graph-data.mjs`) to dynamically generate graph data (`graph-data.json`) from a source of truth (`references.json`).
*   Implement logic in the script to position nodes based on their sequence and identify shared vs. variant terms.
*   Ensure the D3.js visualization uses this dynamically generated data.

**Concepts Covered:**
*   Server-side data generation using Node.js (`fs/promises`).
*   Modular JavaScript (`.mjs` extension for ES Modules).
*   Data structures like `Map` for efficient data processing.
*   Algorithmic thinking for graph layout (e.g., fixed X positions, alternating Y for variants).
*   Updating the `index.html` to load the new `graph-data.json`.

---

## Code Examples (Current State)

*Note: The code provided below reflects the state of the project files after completing Module 2, Lesson 5.*
...
<head>
    ...
    <title>Text Variant Graph - Dynamic Data</title>
...
console.log("Lesson 5: Dynamic Node Positioning and Data Generation")
