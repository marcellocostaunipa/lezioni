# Module 5, Lesson 14: Advanced Styling - Custom Node Shapes and Textures

**Objective:**
*   Implement custom node shapes (e.g., rectangles, diamonds) based on node properties.
*   Apply SVG patterns or textures to nodes for richer visual representation.

**Concepts Covered:**
*   Using SVG `<rect>`, `<polygon>`, etc., instead of just `<circle>`.
*   Conditional rendering of SVG shapes based on data.
*   SVG `<pattern>` elements for defining reusable graphical objects (textures).
*   Applying patterns as `fill` attributes to shapes.

---

## Code Examples (Current State)

*Note: The code provided below reflects the state of the project files after completing Module 5, Lesson 14.*
...
<head>
    ...
    <title>Text Variant Graph - Custom Nodes</title>
    ...
    <style>
        ...
        .node rect, .node circle, .node polygon { /* Apply styles to all shapes */
            fill: steelblue;
            stroke: #fff;
            stroke-width: 1.5px;
        ...
        .node.start rect, .node.end rect { /* Use rect for start/end */
            fill: #4a5568;
        }
        .node.shared circle { /* Keep circle for shared */
            fill: #000000;
        }
        .node.variant polygon { /* Use polygon for variant */
            fill: #e53e3e;
        }
