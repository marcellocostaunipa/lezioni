# Module 5, Lesson 15: Exporting and Sharing - SVG and PNG Export

**Objective:**
*   Add functionality to export the current graph view as an SVG file.
*   Add functionality to export the current graph view as a PNG image.

**Concepts Covered:**
*   Programmatically creating and downloading files in JavaScript.
*   Converting SVG to Data URL for image export.
*   Using `Blob` and `URL.createObjectURL` for file downloads.
*   Leveraging `DOMURL.createObjectURL` and `Image` objects for PNG conversion.
*   Handling asynchronous operations for image loading.

---

## Code Examples (Current State)

*Note: The code provided below reflects the state of the project files after completing Module 5, Lesson 15.*
...
<head>
    ...
    <title>Text Variant Graph - Export</title>
...
<body>
    ...
    <div class="controls-container">
        ...
        <div class="export-buttons">
            <button onclick="window.exportGraph('svg')">Export as SVG</button>
            <button onclick="window.exportGraph('png')">Export as PNG</button>
        </div>
    </div>
...
console.log("Lesson 15: Exporting and Sharing - SVG and PNG Export")
...
window.renderGraph = (data) => {
  ...
  // Define patterns for node textures
  defs.append("pattern")
  ...
  nodes.each(function(d) {
    ...
    const nodeSize = 15; // Base size for shapes
    ...
    if (d.type === "start" || d.type === "end") {
      nodeGroup.append("rect")
        ...
        .attr("ry", 5)
        .attr("fill", d.type === "start" ? witnessColors.start : witnessColors.end);
    } else if (d.type === "variant") {
      nodeGroup.append("polygon")
        .attr("points", `0,-${nodeSize} ${nodeSize},0 0,${nodeSize} -${nodeSize},0`) // Diamond shape
        .attr("fill", witnessColors.variant);
    } else { // Default to circle for 'shared'
      nodeGroup.append("circle")
        .attr("r", nodeSize)
        .attr("fill", witnessColors.shared);
    }
  });

  // Append text to each node group (after shapes, so text is on top)
  nodes.append("text")
    .text(d => d.text)
    .attr("dy", "0.35em"); // Adjust vertical alignment for text
  ...
  console.log("Graph with custom node shapes and textures rendered!");
}
...
window.toggleWitness = (witness, clickedElement) => {
  ...
  svg
    .selectAll(".node circle, .node rect, .node polygon") // Select all node shapes
...
window.resetHighlight = (clickedElement) => {
  ...
  svg.selectAll(".node circle, .node rect, .node polygon").classed("dimmed", false) // Select all node shapes
...
// Function to export the graph (made global)
window.exportGraph = (format) => {
    ...
    // Create a deep clone of the SVG element to avoid modifying the live DOM
    const clonedSvgElement = svgElement.cloneNode(true);

    // Apply current transform to the cloned SVG's content group
    const graphGroup = clonedSvgElement.querySelector(".graph-group");
    if (graphGroup && currentTransform) {
        graphGroup.setAttribute("transform", currentTransform.toString());
    }

    // Remove any dimmed classes or tooltips from the cloned SVG
    clonedSvgElement.querySelectorAll(".dimmed").forEach(el => {
        el.style.opacity = 1; // Make visible for export
        el.classList.remove("dimmed");
    });
    clonedSvgElement.querySelectorAll(".tooltip").forEach(el => el.remove());
    ...
    downloadLink.download = `text-variant-graph.${format}`;
    ...
    if (format === "svg") {
        ...
        const img = new Image();
        img.crossOrigin = "anonymous"; // Important for CORS if SVG contains external resources
        img.onload = () => {
            const canvas = document.createElement("canvas");
            // Set canvas dimensions to match the SVG's intrinsic dimensions for high quality
            canvas.width = svgWidth;
            canvas.height = svgHeight;
            const ctx = canvas.getContext("2d");

            // Draw a white background for the PNG
            ctx.fillStyle = "#ffffff";
            ctx.fillRect(0, 0, canvas.width, canvas.height);

            ctx.drawImage(img, 0, 0, svgWidth, svgHeight);
            URL.revokeObjectURL(svgUrl); // Clean up SVG object URL
            ...
            canvas.toBlob((blob) => {
                ...
                downloadLink.click();
                document.body.removeChild(downloadLink);
                URL.revokeObjectURL(pngUrl); // Clean up PNG object URL
            }, "image/png");
        };
        img.onerror = (error) => {
